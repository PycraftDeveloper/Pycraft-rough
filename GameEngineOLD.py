if __name__ != "__main__":
    ...
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported os")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported traceback")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported datetime")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported platform")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported json")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported pygame")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported pyautogui")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported random")
    
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported ModuleUtils")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported LoggingUtils")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported ErrorUtils")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported FileUtils")
    shared_data.mod_logging_utils__.create_log_message.update_log_information(shared_data, "Imported Registry")

    shared_data.mod_module_utils__.initialize_modules.initialize_external_modules(shared_data)
    shared_data.mod_module_utils__.initialize_modules.initialize_internal_modules(shared_data)

    shared_data.clock = shared_data.mod_pygame__.time.Clock()
    shared_data.fullscreen_x = shared_data.mod_pyautogui__.size()[0]
    shared_data.fullscreen_y = shared_data.mod_pyautogui__.size()[1]
    ...
    class GameEngine(shared_data.mod_base__.CameraWindow):
        ...
        def __init__(self, **kwargs):
            try:
                ...
                WindowSize = self.shared_data.real_window_width, self.shared_data.real_window_height
                CurrentWindowSize = WindowSize

                self.StartLoading = self.shared_data.mod_time__.perf_counter()

                GameLoadProgressPercent = 10

                GameEngine_Initialisation = True
                self.shared_data.mod_game_engine__.CreateEngine.RenderLoaddisplay(self.shared_data)
                CreateLoadScreen = self.shared_data.mod_threading__.Thread(
                    target=self.shared_data.mod_game_engine__.CreateEngine.RenderLoaddisplay,
                    args=(self.shared_data, ))

                CreateLoadScreen.start()
                CreateLoadScreen.name = "Thread_CreateLoadScreen"
                CreateLoadScreen.join()

                super().__init__(**kwargs)

                if self.shared_data.aa:
                    self.samples = int(str(self.shared_data.aa_quality).split("x")[0])
                else:
                    self.samples = 1

                try:
                    displayPosition = self.shared_data.mod_display_utils__.display_utils.get_display_location(
                        self.shared_data)
                    
                except Exception as Message:
                    log_message = "GameEngine > GameEngine > __init__: "+str(Message)
                    
                    self.shared_data.mod_logging_utils__.create_log_message.update_log_warning(
                        self.shared_data,
                        log_message)
                    
                    CurrentWindowSize = self.window_size
                    displayPosition = (int((self.shared_data.fullscreen_x-CurrentWindowSize[0])/2),
                                        int((self.shared_data.fullscreen_y-CurrentWindowSize[1])/2))
                ...
                if self.shared_data.fullscreen:
                    self.wnd.position = displayPosition
                    self.shared_data.mod_base__.CameraWindow.resize(
                        self,
                        self.wnd.size[0],
                        self.wnd.size[1])
                ...
                self.offscreen_depth.filter = (self.shared_data.mod_ModernGL__.LINEAR,
                                               self.shared_data.mod_ModernGL__.LINEAR)

                self.offscreen = self.ctx.framebuffer(depth_attachment=self.offscreen_depth,)

                self.sun_radius = 50

                self.sun = self.shared_data.mod_ModernGL_window_geometry.sphere(
                    radius=self.sun_radius)

                self.moon = self.shared_data.mod_ModernGL_window_geometry.sphere(
                    radius=80)

                self.rain_particle = self.shared_data.mod_ModernGL_window_geometry.sphere(
                    radius=1)

                GameLoadProgressPercent = 30

                # Debug geometry
                self.offscreen_quad = self.shared_data.mod_ModernGL_window_geometry.quad_2d(
                    size=(0.5, 0.5),
                    pos=(0.75, 0.75))

                self.offscreen_quad2 = self.shared_data.mod_ModernGL_window_geometry.quad_2d(
                    size=(0.5, 0.5),
                    pos=(0.25, 0.75))

                self.SkySphere = self.shared_data.mod_ModernGL_window_geometry.sphere(
                    radius=self.skybox_distance)

                self.objects: self.shared_data.mod_typing__.Dict[
                    str,
                    self.shared_data.mod_ModernGL__.VertexArray] = {}

                self.objects_shadow: self.shared_data.mod_typing__.Dict[
                    str,
                    self.shared_data.mod_ModernGL__.VertexArray] = {}

                if self.shared_data.platform == "Linux":
                    # Use 'u' to do things with UVs that this really needs
                    self.scene: self.shared_data.mod_ModernGL_window_.scene.Scene = self.load_scene(
                        self.shared_data.mod_OS__.path.join(
                            self.shared_data.base_folder,
                ...
                # Programs
                self.shared_data.mod_game_engine_utils__.LoadPrograms.LoadProgramFiles(self)

                self.shared_data.mod_game_engine_utils__.LoadPrograms.LoadProgramText(self)
                ...
                if self.shared_data.fancy_particles:
                ...
                self.vbo1.write(
                    self.shared_data.mod_numpy__.fromiter(
                        self.shared_data.mod_game_engine_utils__.Particles.gen_particles(
                            self,
                ...
                self.particles_screen["projection"].write(
                    self.shared_data.mod_game_engine_utils__.Particles.projection(
                        self))
                ...
                SHADOW_SIZE: self.shared_data.mod_typing_Final[int] = 2 << 7
                ...
                self.sampler_depth = self.ctx.sampler(
                    filter=(self.shared_data.mod_ModernGL__.LINEAR,
                            self.shared_data.mod_ModernGL__.LINEAR),
                    compare_func=">=",
                    repeat_x=False,
                    repeat_y=False,)
                ...
                CloudData = self.shared_data.mod_game_engine_utils__.ComputeWeather.ComputeCloudNoise(
                    self,
                    size)

                self.range = self.shared_data.mod_numpy__.max(CloudData) - self.shared_data.mod_numpy__.min(CloudData)

                self.CloudsProgram["CloudHeight"] = 500.0
                self.CloudsProgram["height_max"] = self.range
                self.CloudsProgram["render_fog"] = self.shared_data.render_fog

                self.shadowmap["render_fog"] = self.shared_data.render_fog

                vertices, index = self.shared_data.mod_game_engine_utils__.ComputeWeather.ComputeCloudModel(
                    self,
                    size[0])
                ...
                if self.shared_data.fullscreen is False:
                ...
                self.shared_data.mod_globals__.Share.initialize_controller_game(self)
                ...
                self.shared_data.mod_game_engine_utils__.ComputeWeather.ComputeWeather(self)

                self.WeatherTime = 0
                self.Thundertimer = 0
                self.Lightningtimer = 0
                self.show_strobe_effects = False
                self.Thundertimer_Target = self.shared_data.mod_random__.randint(15, 30)
                switchWeather = self.shared_data.mod_random__.randint(60, 120)
                self.Flashtimer = 0
                ...
                while self.Running:
                    ...
                    if self.Inventory or self.Map:
                        self.shared_data.mod_game_engine_utils__.AccessOtherGUIs.AccessGUI(self)

                    if self.shared_data.mod_pygame__.mixer.music.get_busy():
                        self.shared_data.mod_pygame__.mixer.music.pause()

                    self.shared_data.game_engine_variables = self
                    ...
                    self.shared_data.aFPS += self.shared_data.eFPS
                    self.shared_data.iteration += 1

                    if self.shared_data.detailed_captions:
                        self.shared_data.mod_caption_utils__.generate_captions.GetOpenGLCaption(
                            shared_data,
                            self)

                    self.shared_data.total_move_x = 0
                    self.shared_data.total_move_y = 0
                    self.shared_data.total_move_z = 0
                    ...
                    if self.frametime > 0:
                        self.shared_data.eFPS = 1/self.frametime

                    self.Time_Percent = ((100/1056)*(self.GameTime))

                    self.shared_data.mod_game_engine_utils__.ShadowmappingMathematics.ComputeCelestialEntities(
                        self)

                    self.ctx.enable(
                        self.shared_data.mod_ModernGL__.DEPTH_TEST |
                        self.shared_data.mod_ModernGL__.CULL_FACE)

                    # --- PASS 1: Render shadow map
                    self.shared_data.mod_game_engine_utils__.ShadowmappingMathematics.ComputeShadows(
                        self)

                    # --- PASS 2: Render scene to screen
                    self.wnd.use()
                    cam = self.camera.matrix
                    self.shared_data.x = int(self.camera.position.x*100)/100
                    self.shared_data.y = int(self.camera.position.y*100)/100
                    self.shared_data.z = int(self.camera.position.z*100)/100

                    cam[3][0] = 0
                    cam[3][1] = 0
                    cam[3][2] = 0

                    translation = self.shared_data.mod_pyrr_Matrix44_.from_translation(
                        (0.0, 0.0, 0.0),
                        dtype="f4")
                    ...
                    self.particles_screen["projection"].write(self.shared_data.mod_game_engine_utils__.Particles.projection(self))
                    ...
                    self.shared_data.mod_game_engine_utils__.ComputeWeather.BlendWeather(
                        self)

                    if self.Thundertimer > self.Thundertimer_Target:
                        self.Thundertimer_Target = self.shared_data.mod_random__.randint(5, 30)
                        self.Thundertimer = 0
                        self.Flashtimer = 0
                        self.Lightningtimer = 0
                        if self.shared_data.show_strobe_effects:
                            self.color.value = (1.0, 1.0, 1.0)
                        self.show_strobe_effects = True
                        self.ShowFlash = True
                        if self.shared_data.sound:
                            self.shared_data.mod_sound_utils__.play_sound.play_thunder_sound(
                                self.shared_data)

                    if self.weather == "rain.heavy.thundery":
                        LightningDelay = self.shared_data.mod_random__.randint(30, 75)/100
                    ...
                    if self.weather != "sunny":
                        self.shared_data.mod_pygame__.mixer.Channel(4).unpause()
                        if self.weather == "rain.light":
                            RandomiseVolumeReduction = self.shared_data.mod_random__.randint(2, 5)
                            self.shared_data.mod_pygame__.mixer.Channel(2).set_volume(
                                (self.shared_data.sound_volume/100)/RandomiseVolumeReduction)

                            self.shared_data.mod_pygame__.mixer.Channel(4).set_volume(
                                (((self.shared_data.sound_volume/100)*10)/100))

                        else:
                            self.shared_data.mod_pygame__.mixer.Channel(2).pause()
                            if self.weather == "rain.heavy.thundery":
                                self.shared_data.mod_pygame__.mixer.Channel(4).set_volume(
                                    (((self.shared_data.sound_volume/100)*30)/100)
                                )
                            else:
                                self.shared_data.mod_pygame__.mixer.Channel(4).set_volume(
                                    (((self.shared_data.sound_volume/100)*20)/200)
                                )

                    else:
                        self.shared_data.mod_pygame__.mixer.Channel(4).pause()
                        self.shared_data.mod_pygame__.mixer.Channel(2).set_volume(
                            self.shared_data.sound_volume/100)

                        self.shared_data.mod_pygame__.mixer.Channel(2).unpause()

                    if self.weather != "sunny":
                        self.ctx.enable(self.shared_data.mod_ModernGL__.BLEND)

                    self.SkyBox_texture_Sun.use(location=0)
                    self.SkySphere.render(self.skysphere_prog)

                    if self.weather != "sunny":
                        self.ctx.disable(self.shared_data.mod_ModernGL__.BLEND)
                    ...
                    if self.weather != "sunny":
                        ...
                        self.shared_data.mod_game_engine_utils__.Particles.emit_gpu(
                            self,
                            self.time,
                            self.frametime)

                    self.CloudPos = self.shared_data.mod_pyrr_Vector3_(
                    ...
                    self.CloudsProgram["X_Offset"] = self.shared_data.mod_math__.sin(self.time/50)/10
                    self.CloudsProgram["Y_Offset"] = self.shared_data.mod_math__.cos(self.time/50)/10

                    self.cloud_texture.use(location=0)

                    self.ctx.enable(self.shared_data.mod_ModernGL__.BLEND)

                    self.vao.render(mode=self.shared_data.mod_ModernGL__.TRIANGLE_STRIP)

                    self.ctx.blend_func = self.shared_data.mod_ModernGL__.DEFAULT_BLENDING
                    ...
                    if self.weather == "rain.heavy.thundery" and self.LengthenStorm:
                        switchWeather = switchWeather*(self.shared_data.mod_random__.randint(
                            15,
                            30)/10)
                        self.LengthenStorm = False

                    self.shared_data.mod_game_engine_utils__.OnscreenEventFunction.game_events(self)
                    ...
                    if (Message != "GL_NO_ERROR" and Message != "GL_INVALID_OPERATION"):
                        try:
                            ...
                            self.shared_data.error_message_detailed = self.shared_data.error_message
                ...
                self.shared_data.from_play = True
                ...
                self.mod_pygame__.display.iconify()
                self.shared_data.mod_main__.Initialize.menu_selector(self)
                
            except Exception as Message:
                self.shared_data.mod_game_engine_utils__.Crash.CreateReport(self, Message)
        ...
        def RenderLoaddisplay(self):
            try:
                ...
                while GameEngine_Initialisation:
                    ...
                    self.mod_game_engine__.CreateEngine.GenerateLoaddisplay(
                        self,
                        LoadingFont,
                        text,
                        SecondaryFont,
                    ...
                    tempFPS = self.mod_display_utils__.display_utils.get_play_status(self)
